 ...
 */
 ...
 @Component({
 	...
 	templateUrl: './carcleaning.html',
 	styleUrls: [ '../../style.css' ],
 	providers: [OrderService,MainNavComponent ],
 	animations: animations
 })
 export class CarCleaningBooking extends BookingComponent {
...
public vendornotAvil:boolean=false;
 	...
 	constructor(public titleService:Title, public fb: FormBuilder ,private localityService:LocalityService,
 		private bookingService:BookingService,private orderService:OrderService, private userService: UserService,
 		private _sanitizer: DomSanitizer) {
 		super(titleService,'Service Sathi - Car Cleaner Booking',6);
 		this.stageStates[0] = true;
 		this.expertise = 'Car Cleaner';
 		this.expertiselink = 'car_cleaning';
 		this.city = 'Raipur';
 		...
 		this.date="";
 		//this.expid = '5884e5a9ccadb341068ef274';
		...
		this.localityService.serviceDiscription("Car Cleaner").subscribe(
       ...
       descriptions => {
 		...
 		this.requestForm = this.fb.group({
 			'jobs': this.fb.group({
 				interior : [false, Validators.required ],
 				exterior : [false, Validators.required ],
 	...
 	formChanged( data : any ) : void {
 		...
 		switch(this.stage){
 			case 2 : 
 			if(!this.atleastOneJob(data.jobs)){
 	...
 	atleastOneJob(jobs:any) : boolean {
 		let count = 0;
 		if(jobs.interior){
 			count++;
 		}
...
citymodelDissmis() : void{
    ...
    Dissmis():void{
 	...
 	book() : void{
        ...
        bookingData.serviceName ='Car Cleaner';
	...
	onSclectTime=($event: any)=>  {
     ...
     localStorage.setItem("getVendor", JSON.stringify({'locality':locality,"serviceName":"Car Cleaner"}));
